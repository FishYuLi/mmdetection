...
class MeanShift(object):
    def __init__(self, bandwidth, stop_thr=1e-4, clus_thr=1e-3):
        ...
        self.cons_plus = torch.from_numpy(
            np.array([1 / (bandwidth * np.sqrt(2 * np.pi))])).cuda().float()
    ...
    def mean_shift(self, points, stay):
        ...
        while c > 0:
            ...
            left_idx = (shift > self.stop_thr).nonzero(as_tuple=True)[
                0]  # (newc, )
            save_idx = (shift <= self.stop_thr).nonzero(as_tuple=True)[
                0]  # (c - newc, )
            ...
            corresponding_points.append(
                points_copy.index_select(dim=0, index=save_idx))
    ...
    def _cluster_points(self, shift_points):
        ...
        for i in range(n):
            ...
            same_cluster_idx = \
                (cur_dist < self.clus_thr).nonzero(as_tuple=True)[0]
